  ...
  namespace gate
  {
    ...
    namespace clear_detail
    {
      ...
      -> void
      {
        ...
        auto const multiplier = pow(std::accumulate(begin(zero_probabilities), end(zero_probabilities), real_type{0}), -half<real_type>());

        ::ket::utility::loop_n(
          parallel_policy, static_cast<StateInteger>(last - first)/2u,
          [first, lower_bits_mask, upper_bits_mask, multiplier](StateInteger const value_wo_qubit, int const)
          {
            // xxxxx0xxxxxx
            auto const zero_index = ((value_wo_qubit bitand upper_bits_mask) << 1u) bitor (value_wo_qubit bitand lower_bits_mask);
