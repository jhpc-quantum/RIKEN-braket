...
# ifndef KET_USE_BIT_MASKS_EXPLICITLY
      ...
      class cache_aware_iterator
      {
       ...
       public:
        ...
        {
          auto const chunk_index = static_cast<state_integer_type>(index_) / chunk_size;
          auto const index_in_chunk = static_cast<state_integer_type>(index_) % chunk_size;
          return *(first
                   + ::ket::gate::utility::index_with_qubits(
                       tag_index_wo_qubits_, chunk_index,
                       unsorted_tag_qubits_first_, unsorted_tag_qubits_last_,
                       sorted_tag_qubits_with_sentinel_first_, sorted_tag_qubits_with_sentinel_last_) * chunk_size
                   + index_in_chunk);
        }

        auto operator[](std::size_t const n) -> reference
        {
          auto const chunk_index = static_cast<state_integer_type>(index_ + static_cast<difference_type>(n)) / chunk_size;
          auto const index_in_chunk = static_cast<state_integer_type>(index_ + static_cast<difference_type>(n)) % chunk_size;
          return *(first
                   + ::ket::gate::utility::index_with_qubits(
                       tag_index_wo_qubits_, chunk_index,
                       unsorted_tag_qubits_first_, unsorted_tag_qubits_last_,
        ...
        auto operator++() noexcept -> cache_aware_iterator& { ++index_; }
        auto operator++(int) noexcept -> cache_aware_iterator { auto result = *this; ++*this; return result; }
        auto operator--() noexcept -> cache_aware_iterator& { --index_; }
        auto operator--(int) noexcept -> cache_aware_iterator { auto result = *this; --*this; return result; }
        auto operator+=(difference_type const n) noexcept -> cache_aware_iterator& { index_ += n; }
        auto operator-=(difference_type const n) noexcept -> cache_aware_iterator& { index_ -= n; }
...
# else // KET_USE_BIT_MASKS_EXPLICITLY
      ...
      class cache_aware_iterator
      {
       ...
       public:
        ...
        auto operator*() const noexcept -> reference
        {
          auto const chunk_index = static_cast<StateInteger>(index_) / chunk_size;
          auto const index_in_chunk = static_cast<StateInteger>(index_) % chunk_size;
          return *(first
                   + ::ket::gate::utility::index_with_qubits(
                       tag_index_wo_qubits_, chunk_index,
                       tag_qubit_masks_first_, tag_qubit_masks_last_,
                       tag_index_masks_first_, tag_index_masks_last_) * chunk_size
                   + index_in_chunk);
        }

        auto operator[](std::size_t const n) -> reference
        {
          auto const chunk_index = static_cast<StateInteger>(index_ + static_cast<difference_type>(n)) / chunk_size;
          auto const index_in_chunk = static_cast<StateInteger>(index_ + static_cast<difference_type>(n)) % chunk_size;
          return *(first
                   + ::ket::gate::utility::index_with_qubits(
                       tag_index_wo_qubits_, chunk_index,
                       tag_qubit_masks_first_, tag_qubit_masks_last_,
