...
# endif // BOOST_NO_CXX14_GENERIC_LAMBDAS
            ...
            { throw ::ket::mpi::gate::page::unsupported_page_gate_operation<0>{"controlled_phase_shift_coeff_cp_tu"}; }
              ...
              ::ket::qubit<StateInteger, BitInteger> const least_permutated_unit_qubit)
            {
              ...
              auto const target_qubit_mask
                = StateInteger{1u}
                  << (permutation[target_qubit] - least_permutated_unit_qubit);
              ...
              for (auto data_block_index = StateInteger{0u}; data_block_index < num_data_blocks; ++data_block_index)
              {
                ...
                if ((unit_qubit_value bitand target_qubit_mask) == StateInteger{0u})
                  continue;
                ...
                auto const control_qubit_mask
                  = ::ket::utility::integer_exp2<StateInteger>(
                      permutation[control_qubit.qubit()] - static_cast<BitInteger>(num_nonpage_local_qubits));
                auto const lower_bits_mask = control_qubit_mask - StateInteger{1u};
                ...
                for (auto page_index_wo_qubit = std::size_t{0u}; page_index_wo_qubit < num_pages / 2u; ++page_index_wo_qubit)
                {
                  ...
                  // x1x
                  auto const one_page_index = zero_page_index bitor control_qubit_mask;
...
# endif // BOOST_NO_CXX14_GENERIC_LAMBDAS
              ...
              StateInteger const global_qubit_value)
            {
              ...
              auto const target_qubit_mask
                = StateInteger{1u}
                  << (permutation[target_qubit] - least_permutated_global_qubit);

              if ((global_qubit_value bitand target_qubit_mask) == StateInteger{0u})
                return local_state;
