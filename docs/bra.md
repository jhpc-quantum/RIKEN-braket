...
* `XXXXXX i j k l m n` or `X6 i j k l m n`: the Pauli $\hat{X}$ gates operated on qubits $i$, ..., $n$. If you use two qubits, use `XX i j` or `X2 i j` instead. The Pauli $\hat{Y}$ and $\hat{Z}$ versions are also supported.
* `CCCXXX c1 c2 c3 t1 t2 t3` or `C3X3 c1 c2 c3 t1 t2 t3`: the controlled Pauli $\hat{X}$ gates. Qubits $c_1$, $c_2$, $c_3$ are control qubits, and qubits $t_1$, $t_2$, and $t_3$ are target qubits. If you use two target qubits and two control qubits, use `CCXX c1 c2 t1 t2` or `C2X2 c1 c2 t1 t2` instead. The Pauli $\hat{Y}$ and $\hat{Z}$ versions are also supported.
...
* `SX i`: the square-root Pauli $\sqrt{\hat{X}}$ gate operated on qubit $i$, $\sqrt{\hat{X}} (a_0 \ket{0} + a_1 \ket{1}) = \biggl( \frac{1 + \mathrm{i}}{2} a_0 + \frac{1 - \mathrm{i}}{2} a_1 \biggr) \ket{0} + \biggl( \frac{1 - \mathrm{i}}{2} a_0 + \frac{1 + \mathrm{i}}{2} a_1 \biggr) \ket{1}$
* `SX+ i`: the square-root Pauli $\sqrt{\hat{X}}^\dagger$ gate operated on qubit $i$, $\sqrt{\hat{X}}^\dagger (a_0 \ket{0} + a_1 \ket{1}) = \biggl( \frac{1 - \mathrm{i}}{2} a_0 + \frac{1 + \mathrm{i}}{2} a_1 \biggr) \ket{0} + \biggl( \frac{1 + \mathrm{i}}{2} a_0 + \frac{1 - \mathrm{i}}{2} a_1 \biggr) \ket{1}$
* `SY i`: the square-root Pauli $\sqrt{\hat{Y}}$ gate operated on qubit $i$, $\sqrt{\hat{Y}} (a_0 \ket{0} + a_1 \ket{1}) = \biggl( \frac{1 + \mathrm{i}}{2} a_0 - \frac{1 + \mathrm{i}}{2} a_1 \biggr) \ket{0} + \biggl( \frac{1 + \mathrm{i}}{2} a_0 + \frac{1 + \mathrm{i}}{2} a_1 \biggr) \ket{1}$
* `SY+ i`: the square-root Pauli $\sqrt{\hat{Y}}^\dagger$ gate operated on qubit $i$, $\sqrt{\hat{Y}}^\dagger (a_0 \ket{0} + a_1 \ket{1}) = \biggl( \frac{1 - \mathrm{i}}{2} a_0 + \frac{1 - \mathrm{i}}{2} a_1 \biggr) \ket{0} + \biggl( -\frac{1 - \mathrm{i}}{2} a_0 + \frac{1 - \mathrm{i}}{2} a_1 \biggr) \ket{1}$
* `SZ i`: the square-root Pauli $\sqrt{\hat{Z}}$ gate operated on qubit $i$, $\sqrt{\hat{Z}} (a_0 \ket{0} + a_1 \ket{1}) = a_0 \ket{0} + \mathrm{i} a_1 \ket{1}$
* `SZ+ i`: the square-root Pauli $\sqrt{\hat{Z}}^\dagger$ gate operated on qubit $i$, $\sqrt{\hat{Z}}^\dagger (a_0 \ket{0} + a_1 \ket{1}) = a_0 \ket{0} - \mathrm{i} a_1 \ket{1}$
* `CCCCCSX c1 c2 c3 c4 c5 t` or `C5SX c1 c2 c3 c4 c5 t`: the controlled $\sqrt{\hat{X}}$ gate. Qubits $c_1$, ..., $c_5$ are control qubits, and qubit $t$ is a target qubit. If you use three control qubits, use `CCCSX c1 c2 c3 t` or `C3SX c1 c2 c3 t` instead. Similar instructions with control qubits for `SX+`, `SY`, `SY+` are also supported.
* `CCCSZZZ c1 c2 c3 t1 t2 t3` or `C3SZ3 c1 c2 c3 t1 t2 t3`: the controlled $\sqrt{\hat{Z}}$ gate. Qubits $c_1$, ..., $c_3$ are control qubits, and qubits $t_1$, ..., $t_3$ is target qubits. If you use two target qubits and two control qubits, use `CCSZZ c1 c2 t1 t2` or `C2SZ2 c1 c2 t1 t2` instead. Similar instructions with control qubits for `SZ+` are also supported.
* `S i`: the $\hat{S}$ gate operated on qubit $i$. Note that `S` is an alias of `SZ`.
* `S+ i`: the $\hat{S}^\dagger$ gate operated on qubit $i$. Note that `S+` is an alias of `SZ+`.
...
* `CCCCCS c1 c2 c3 c4 c5 t` or `C5S c1 c2 c3 c4 c5 t`: the controlled $\hat{S}$ gate. Qubits $c_1$, ..., $c_5$ are control qubits, and qubit $t$ is a target qubit. If you use three control qubits, use `CCCS c1 c2 c3 t` or `C3S c1 c2 c3 t` instead. The $\hat{S}^\dagger$, $\hat{T}$, and $\hat{T}^\dagger$ versions are also supported.
...
* `CCCCCU1 c1 c2 c3 c4 c5 t lambda` or `C5U1 c1 c2 c3 c4 c5 t lambda`: the controlled phase-shift gate. Qubits $c_1$, ..., $c_5$ are control qubits, and qubit $t$ is a target qubit. If you use three control qubits, use `CCCU1 c1 c2 c3 t lambda` or `C3U1 c1 c2 c3 t lambda` instead. The $\hat{U}_2$ and $\hat{U}_3$ versions are also supported.
...
* `CCCCCR c1 c2 c3 c4 c5 t k` or `C5R c1 c2 c3 c4 c5 t k`: the controlled phase-shift gate which changes the phase of target qubit $t$ by an angle $2\pi/2^k$. Note that qubits $c_1$, ..., $c_5$ are control qubits. If you use three control qubits, use `CCCR c1 c2 c3 t k` or `C3R c1 c2 c3 t k` instead. The `R+` version is also supported.
...
* `EXXXXXX i j k l m n theta` or `EX6 i j k l m n theta`: the exponential Pauli $\hat{X}$ gates operated on qubits $i$, ..., $n$. If you use two qubits, use `EXX i j theta` or `EX2 i j theta` instead. The Pauli $\hat{Y}$ and $\hat{Z}$ versions are also supported.
* `CCCEXXX c1 c2 c3 t1 t2 t3 theta` or `C3EX3 c1 c2 c3 t1 t2 t3 theta`: the controlled exponential Pauli $\hat{X}$ gates. Qubits $c_1$, $c_2$, $c_3$ are control qubits, and qubits $t_1$, $t_2$, and $t_3$ are target qubits. If you use two target qubits and two control qubits, use `CCEXX c1 c2 t1 t2 theta` or `C2EX2 c1 c2 t1 t2 theta` instead. The Pauli $\hat{Y}$ and $\hat{Z}$ versions are also supported.
