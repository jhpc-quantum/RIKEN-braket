...
* `VAR name type [size]`: declares classical variable/array whose name is `name`, type is `type`, and size is `size`. Possible `type`'s are `REAL` or `INT`. If `size` is not specified, array size is assumed to be 1. Note that classical variable is just an array whose size is 1. In order to get an element of array `XS`, specify such as `XS:0`, which corresponds to `X[0]` in C/C++. In this quantum assembly language, `A:B:C:D` is for example a valid expression, and corresponding to `A[B[C[D]]]` in C/C++.
