#ifndef BRA_FUSED_GATE_FUSED_ADJ_MULTI_CONTROLLED_SQRT_PAULI_ZN_HPP
# define BRA_FUSED_GATE_FUSED_ADJ_MULTI_CONTROLLED_SQRT_PAULI_ZN_HPP
...
namespace bra
{
  namespace fused_gate
  {
    ...
    class fused_adj_multi_controlled_sqrt_pauli_zn final
      : public ::bra::fused_gate::fused_gate<Iterator>
    {
     ...
     public:
      explicit fused_adj_multi_controlled_sqrt_pauli_zn(std::vector< ::bra::qubit_type > const& target_qubits, std::vector< ::bra::control_qubit_type > const& control_qubits);
      explicit fused_adj_multi_controlled_sqrt_pauli_zn(std::vector< ::bra::qubit_type >&& target_qubits, std::vector< ::bra::control_qubit_type > const& control_qubits);
      explicit fused_adj_multi_controlled_sqrt_pauli_zn(std::vector< ::bra::qubit_type > const& target_qubits, std::vector< ::bra::control_qubit_type >&& control_qubits);
      explicit fused_adj_multi_controlled_sqrt_pauli_zn(std::vector< ::bra::qubit_type >&& target_qubits, std::vector< ::bra::control_qubit_type >&& control_qubits);

      ~fused_adj_multi_controlled_sqrt_pauli_zn() = default;
      fused_adj_multi_controlled_sqrt_pauli_zn(fused_adj_multi_controlled_sqrt_pauli_zn const&) = delete;
      fused_adj_multi_controlled_sqrt_pauli_zn& operator=(fused_adj_multi_controlled_sqrt_pauli_zn const&) = delete;
      fused_adj_multi_controlled_sqrt_pauli_zn(fused_adj_multi_controlled_sqrt_pauli_zn&&) = delete;
      fused_adj_multi_controlled_sqrt_pauli_zn& operator=(fused_adj_multi_controlled_sqrt_pauli_zn&&) = delete;
...
# ifndef KET_USE_BIT_MASKS_EXPLICITLY
#   define CALL_ADJ_SQRT_PAULI_Z(z, num_control_qubits, num_target_qubits) \
    template <std::size_t num_fused_qubits>\
    auto call_adj_sqrt_pauli_z_ ## num_target_qubits ## _ ## num_control_qubits(\
      Iterator const first, ::bra::state_integer_type const fused_index_wo_qubits,\
...
#   define CALL_ADJ_SQRT_PAULI_Z(z, num_control_qubits, num_target_qubits) \
    ...
    auto call_adj_sqrt_pauli_z_ ## num_target_qubits ## _ ## num_control_qubits(\
      ...
      std::vector< ::bra::control_qubit_type > const& control_qubits) const -> void;
# endif // KET_USE_BIT_MASKS_EXPLICITLY
# define DO_CALL_ADJ_SQRT_PAULI_Z(z, num_target_qubits, _) \
BOOST_PP_REPEAT_FROM_TO(0, BOOST_PP_INC(BOOST_PP_SUB(BRA_MAX_NUM_FUSED_QUBITS, num_target_qubits)), CALL_ADJ_SQRT_PAULI_Z, num_target_qubits)
BOOST_PP_REPEAT_FROM_TO(1, BOOST_PP_INC(BRA_MAX_NUM_FUSED_QUBITS), DO_CALL_ADJ_SQRT_PAULI_Z, nil)
...
# undef DO_CALL
    ...
    }; // class fused_adj_multi_controlled_sqrt_pauli_zn<Iterator>
  } // namespace fused_gate
} // namespace bra


#endif // BRA_FUSED_GATE_FUSED_ADJ_MULTI_CONTROLLED_SQRT_PAULI_ZN_HPP
