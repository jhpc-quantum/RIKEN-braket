...
namespace bra
{
  namespace gate
  {
    multi_controlled_sqrt_pauli_zn::multi_controlled_sqrt_pauli_zn(
      ...
      : ::bra::gate::gate{}, target_qubits_{target_qubits}, control_qubits_{control_qubits},
        name_{std::string(control_qubits_.size(), 'C').append("s").append(target_qubits_.size(), 'Z')}
    { }
    ...
    ::bra::state& multi_controlled_sqrt_pauli_zn::do_apply(::bra::state& state) const
    { return state.multi_controlled_sqrt_pauli_zn(target_qubits_, control_qubits_); }

    std::string const& multi_controlled_sqrt_pauli_zn::do_name() const { return name_; }
    std::string multi_controlled_sqrt_pauli_zn::do_representation(
