...
#include <bra/gate/adj_multi_controlled_not.hpp>
...
namespace bra
{
  namespace gate
  {
    adj_multi_controlled_not::adj_multi_controlled_not(
    ...
    ::bra::state& adj_multi_controlled_not::do_apply(::bra::state& state) const
    { return state.adj_multi_controlled_not(target_qubit_, control_qubits_); }

    std::string const& adj_multi_controlled_not::do_name() const { return name_; }
    std::string adj_multi_controlled_not::do_representation(
