  ...
  namespace fused_gate
  {
      ...
      std::vector< ::bra::bit_integer_type > const& to_qubit_index_in_fused_gates) const -> void
    {
      if (unsorted_fused_qubits.size() < std::size_t{1u})
        throw std::runtime_error{"fused_adj_multi_controlled_exponential_swap requires at least one fused qubit"};
      ...
      ::ket::gate::fused::runtime::ranges::adj_exponential_swap(
    ...
    template <typename Iterator>
    auto fused_adj_multi_controlled_exponential_swap<Iterator>::do_call_in_execute(
