  ...
  namespace fused_gate
  {
      ...
      std::vector< ::bra::bit_integer_type > const& to_qubit_index_in_fused_gates) const -> void
    {
      if (unsorted_fused_qubits.size() < std::size_t{1u})
        throw std::runtime_error{"fused_controlled_pauli_y requires at least one fused qubit"};
    ...
    template <typename Iterator>
    auto fused_controlled_pauli_y<Iterator>::do_call_in_execute(
    ...
    template <typename Iterator>
    auto fused_controlled_pauli_y<Iterator>::do_disable_control_qubits(
