...
namespace bra
{
  ...
  unit_mpi_state::unit_mpi_state(
    ...
    : ::bra::state{total_num_qubits, seed, communicator, environment},
      ...
      data_{generate_data(num_local_qubits, initial_integer, communicator, environment)}
  { }
  ...
  unit_mpi_state::data_type unit_mpi_state::generate_data(
    unsigned int const num_local_qubits,
    ::bra::state::state_integer_type const initial_integer,
    yampi::communicator const& communicator, yampi::environment const& environment)
  {
    auto const data_block_size
      = ket::utility::integer_exp2< ::bra::state::state_integer_type >(num_local_qubits);
    auto const num_data_blocks
      = ket::mpi::utility::policy::num_data_blocks(mpi_policy_, communicator, environment);
    auto const data_size = num_data_blocks * data_block_size;

    auto result = unit_mpi_state::data_type(data_size, complex_type{0});

    using ket::mpi::permutate_bits;
    auto const rank_index
      = ket::mpi::utility::qubit_value_to_rank_index(
          mpi_policy_, result, permutate_bits(permutation_, initial_integer),
