...
namespace bra
{
  namespace fused_gate
  {
    ...
    template <typename Iterator>
    [[noreturn]] auto fused_swap<Iterator>::do_call(
...
# define DO_CALL(z, num_fused_qubits, _) \
    ...
    { ::ket::gate::fused::swap(first, fused_index_wo_qubits, unsorted_fused_qubits, sorted_fused_qubits_with_sentinel, qubit1_, qubit2_); }
#else // KET_USE_BIT_MASKS_EXPLICITLY
# define DO_CALL(z, num_fused_qubits, _) \
    template <typename Iterator>\
    auto fused_swap<Iterator>::do_call(\
    ...
    { ::ket::gate::fused::swap(first, fused_index_wo_qubits, qubit_masks, index_masks, qubit1_, qubit2_); }
#endif // KET_USE_BIT_MASKS_EXPLICITLY
...
#if defined(KET_ENABLE_CACHE_AWARE_GATE_FUNCTION) && !defined(KET_USE_ON_CACHE_STATE_VECTOR)
  template class fused_swap<ket::gate::utility::cache_aware_iterator< ::bra::data_type::iterator >>;
# ifndef BRA_NO_MPI
  template class fused_swap<ket::gate::utility::cache_aware_iterator< ::bra::paged_data_type::iterator >>;
# endif // BRA_NO_MPI
#endif // defined(KET_ENABLE_CACHE_AWARE_GATE_FUNCTION) && !defined(KET_USE_ON_CACHE_STATE_VECTOR)
  } // namespace fused_gate
} // namespace bra
