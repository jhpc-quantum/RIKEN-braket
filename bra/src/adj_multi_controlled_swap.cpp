...
#include <bra/gate/adj_multi_controlled_swap.hpp>
...
namespace bra
{
  namespace gate
  {
    adj_multi_controlled_swap::adj_multi_controlled_swap(
    ...
    ::bra::state& adj_multi_controlled_swap::do_apply(::bra::state& state) const
    { return state.adj_multi_controlled_swap(target_qubit1_, target_qubit2_, control_qubits_); }

    std::string const& adj_multi_controlled_swap::do_name() const { return name_; }
    std::string adj_multi_controlled_swap::do_representation(
